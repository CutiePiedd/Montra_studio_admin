...
if (isset($_GET['package_id']) && isset($_GET['filename'])) {
  ...
  // Fetch the package
  $result = mysqli_query($conn, "SELECT images FROM packages_squad WHERE id=$id");
  ...
  if ($package) {
    ...
    // Update DB
    $update = "UPDATE packages_squad SET images='$new_images' WHERE id=$id";
    ...
    header("Location: edit_packages_squad.php?deleted=1");
    exit();
  }
}

header("Location: edit_packages_squad.php?error=1");
exit();
?>
