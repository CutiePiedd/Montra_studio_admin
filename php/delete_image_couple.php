...
if (isset($_GET['package_id']) && isset($_GET['filename'])) {
  ...
  // Fetch the package
  $result = mysqli_query($conn, "SELECT images FROM packages_couple WHERE id=$id");
  ...
  if ($package) {
    ...
    // Update DB
    $update = "UPDATE packages_couple SET images='$new_images' WHERE id=$id";
    ...
    header("Location: edit_packages_couple.php?deleted=1");
    exit();
  }
}

header("Location: edit_packages_couple.php?error=1");
exit();
?>
